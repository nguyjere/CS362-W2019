...
int main() {
	...
	for (int i = 0; i < numOfTests; i++)
	{
		...
		gainCard(smithy, &G, 2, G.whoseTurn);
		...
		// Player plays smithy
		res = cardEffect(smithy, choice1, choice2, choice3, &G, G.handCount[G.whoseTurn] - 1, &bonus);
		...
		assertEqual((finalHandCount - initialHandCount), 3, "Smithy did not yield exactly 3 new cards in hand", &testRes);
		assertNotEqual(G.hand[G.whoseTurn][5], smithy, "Smithy was not removed from player's hand", &testRes);
		assertEqual(G.playedCards[G.playedCardCount - 1], smithy, "Smithy was not moved to played pile", &testRes);
